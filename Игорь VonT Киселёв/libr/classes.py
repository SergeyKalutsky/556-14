...
class Weapon(obj):
    def __init__(self, position, image, size=1, player=None):
        ...
        self.sprite_sheet = SpriteSheet(image, size=size)
        self.realimage = self.sprite_sheet.get_image(0, 0, 12*size, 7*size)
        self.image_size = self.realimage.get_size()
        self.size = size

        #self.realimage = pygame.image.load(image).convert_alpha()
        self.rect = self.realimage.get_rect()
        #self.realimage = pygame.transform.scale(self.realimage, (self.rect.width*size, self.rect.height*size))
        self.rect = self.realimage.get_rect()
        self.image = self.realimage

    def hand(self, player):
        if self.in_hand:
            self.position = [player.position[0]+player.rect.width/4, player.position[1]+player.rect.height/4]
    def rotate(self, angle):
        self.image = pygame.transform.rotate(self.realimage, angle)
        if angle > -90 and angle < 90:
            self.realimage = self.sprite_sheet.get_image(0, 0, 12*self.size, 7*self.size)
        else:
            self.realimage = self.sprite_sheet.get_image(12*self.size, 0, 12*self.size, 7*self.size)
